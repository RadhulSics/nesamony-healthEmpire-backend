...
const viewTrainers=(req,res)=>{
  trainer.find({isactive:true}).exec()
  .then(data=>{
    if(data.length>0){
    res.json({
        status:200,
...
const editTrainerById=(req,res)=>{
    ...
    trainer.findByIdAndUpdate({_id:mongoose.Types.ObjectId(req.body.id)},{
        name:req.body.name,
        gender:req.body.gender,
        age:req.body.age,
  ...
  const viewTrainerById=(req,res)=>{
   ...
   const removeTrainerById=(req,res)=>{
    ...
    trainer.findByIdAndDelete({_id:req.params.id}).exec()
    .then(data=>{
    emps=data
      console.log(data);
      res.json({
...
const viewTrainerRequests=(req,res)=>{
  trainer.find({isactive:false}).exec()
  .then(data=>{
    if(data.length>0){
    res.json({
        status:200,
...
const ApproveTrainer=(req,res)=>{
  ...
  trainer.findByIdAndUpdate({_id:req.params.id},{
    isactive:true
    })
.exec().then(data=>{
  res.json({
...
const forgotPwd=(req,res)=>{
  ...
  trainer.findOneAndUpdate({email:req.body.email},{
...
module.exports={registerTrainer,login,requireAuth,viewTrainers,removeTrainerById,viewTrainerById,editTrainerById,
  upload,viewTrainerRequests,ApproveTrainer,forgotPwd}